...
import com.zcore.core.system.BProcessManagerService;
import com.zcore.core.system.ISystemService;
import com.zcore.core.system.ProcessRecord;
...
import com.zcore.entity.pm.InstalledPackage;
import com.zcore.utils.AbiUtils;
import com.zcore.utils.FileUtils;
import com.zcore.utils.Slog;
import com.zcore.utils.compat.PackageParserCompat;
...
public class BPackageManagerService extends IBPackageManagerService.Stub implements ISystemService {
    ...
    public ApplicationInfo getApplicationInfo(String packageName, int flags, int userId) {
        ...
        if (Objects.equals(packageName, ZCoreCore.getHostPkg())) {
            try {
                return ZCoreCore.getPackageManager().getApplicationInfo(packageName, flags);
    ...
    public List<ResolveInfo> queryIntentServices(
        ...
        final String resolvedType = intent.resolveTypeIfNeeded(ZCoreCore.getContext().getContentResolver());
    ...
    public PackageInfo getPackageInfo(String packageName, int flags, int userId) {
        ...
        if (Objects.equals(packageName, ZCoreCore.getHostPkg())) {
            try {
                return ZCoreCore.getPackageManager().getPackageInfo(packageName, flags);
            } catch (PackageManager.NameNotFoundException e) {
                e.printStackTrace();
            }
            return null;
...
//                }
    ...
    private List<ApplicationInfo> getInstalledApplicationsListInternal(int i, int i2, int i3) {
        ...
        synchronized (this.mPackages) {
            ...
            for (BPackageSettings next : this.mPackages.values()) {
                if (!GmsCore.isGoogleAppOrService(next.pkg.packageName)) {
                    ApplicationInfo generateApplicationInfo = PackageManagerCompat.generateApplicationInfo(next.pkg, i, next.readUserState(i2), i2);
    ...
    public List<InstalledPackage> getInstalledPackagesAsUser(int i) {
        ...
        synchronized (this.mPackages) {
            ...
            for (BPackageSettings next : this.mPackages.values()) {
                if (next.getInstalled(i) && !GmsCore.isGoogleAppOrService(next.pkg.packageName)) {
                    InstalledPackage installedPackage = new InstalledPackage();
                    installedPackage.userId = i;
                    installedPackage.packageName = next.pkg.packageName;
    ...
    private InstallResult installPackageAsUserLocked(String file, InstallOption option, int userId) {
        ...
        try {
            ...
            if (option.isFlag(InstallOption.FLAG_URI_FILE)) {
                ...
                InputStream inputStream = ZCoreCore.getContext().getContentResolver().openInputStream(Uri.parse(file));
            ...
            PackageInfo packageArchiveInfo = ZCoreCore.getPackageManager().getPackageArchiveInfo(apkFile.getAbsolutePath(), 0);
            ...
            if (!support) {
                String msg = packageArchiveInfo.applicationInfo.loadLabel(ZCoreCore.getPackageManager()) + "[" + packageArchiveInfo.packageName + "]";
                return result.installError(packageArchiveInfo.packageName,
                        msg + "\n" + (ZCoreCore.is64Bit() ? "The box does not support 32-bit Application" : "The box does not support 64-bit Application"));
            }
            ...
            if (option.isFlag(InstallOption.FLAG_SYSTEM)) {
                aPackage.applicationInfo = ZCoreCore.getPackageManager().getPackageInfo(aPackage.packageName, 0).applicationInfo;
            }
