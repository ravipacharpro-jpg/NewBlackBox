...
//                    }
        ...
        try {
            base = ZCoreCore.getContext().getPackageManager().getPackageInfo(p.packageName, flags);
    ...
    public static ApplicationInfo generateApplicationInfo(BPackage p, int flags, BPackageUserState state, int userId) {
        ...
        try {
            baseApplication = ZCoreCore.getPackageManager().getApplicationInfo(ZCoreCore.getHostPkg(), flags);
        } catch (Exception e) {
            return null;
        }
        String sourceDir = p.baseCodePath;
        if (p.applicationInfo == null) {
            p.applicationInfo = ZCoreCore.getPackageManager()
...
//            ApplicationInfo base = ZCoreCore.getContext().getApplicationInfo();
