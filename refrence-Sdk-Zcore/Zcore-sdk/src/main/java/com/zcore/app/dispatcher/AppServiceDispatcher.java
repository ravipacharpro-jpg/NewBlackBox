package com.zcore.app.dispatcher;
...
import com.zcore.entity.ServiceRecord;
import com.zcore.entity.UnbindRecord;
import com.zcore.proxy.record.ProxyServiceRecord;
...
public class AppServiceDispatcher {
    ...
    private final Handler mHandler = ZCoreCore.get().getHandler();
...
//        Log.d(TAG, "onStartCommand: " + component.toString());
        ...
        try {
            ...
            ZCoreCore.getBActivityManager().onStartCommand(proxyIntent, stubRecord.mUserId);
            return i;
        } catch (Throwable e) {
            e.printStackTrace();
        }
...
//        Log.d(TAG, "onLowMemory");
    ...
    public boolean onUnbind(Intent proxyIntent) {
        ...
        try {
            UnbindRecord unbindRecord = ZCoreCore.getBActivityManager().onServiceUnbind(proxyIntent, BActivityThread.getUserId());
            ...
            if (destroy || record.decreaseConnectionCount(intent)) {
                ...
                if (destroy) {
                    service.onDestroy();
                    ZCoreCore.getBActivityManager().onServiceDestroy(proxyIntent, BActivityThread.getUserId());
        ...
        if (record.getService() != null) {
            ...
            try {
                if (destroy) {
                    ...
                    ZCoreCore.getBActivityManager().onServiceDestroy(intent, BActivityThread.getUserId());
                    mService.remove(new Intent.FilterComparison(intent));
                }
            } catch (Throwable e) {
                e.printStackTrace();
