...
import com.zcore.utils.DrawableUtils;
...
public class TaskDescriptionCompat {
    public static ActivityManager.TaskDescription fix(ActivityManager.TaskDescription td) {
        ...
        ActivityManager am = (ActivityManager) ZCoreCore.getContext().getSystemService(Context.ACTIVITY_SERVICE);
    ...
    private static CharSequence getApplicationLabel() {
        try {
            PackageManager pm = ZCoreCore.getPackageManager();
    ...
    private static Drawable getApplicationIcon() {
		try {
			PackageManager pm = ZCoreCore.getContext().getPackageManager();
