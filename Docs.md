...
```java
// Using NyxBoxCore API
NyxBoxCore.get().installPackageAsUser(apkFile, userId);

// Example with error handling
try {
    InstallResult result = NyxBoxCore.get().installPackageAsUser(apkFile, 0);
    if (result.isSuccess()) {
...
NyxBoxCore.get().installPackageAsUser("com.example.app", userId);

// Check if package exists first
if (NyxBoxCore.getPackageManager().getPackageInfo("com.example.app", 0) != null) {
    NyxBoxCore.get().installPackageAsUser("com.example.app", userId);
}
```
...
NyxBoxCore.get().installPackageAsUser(apkUri, userId);
```
...
NyxBoxCore.get().uninstallPackage(packageName, userId);

// Force uninstall if needed
NyxBoxCore.get().uninstallPackage(packageName, userId, true);
```

#### Clean App Data
```java
// Clear app data without uninstalling
NyxBoxCore.get().clearAppData(packageName, userId);

// Clear specific data types
NyxBoxCore.get().clearAppData(packageName, userId, "cache");
NyxBoxCore.get().clearAppData(packageName, userId, "data");
```
...
List<AppInfo> virtualApps = NyxBoxCore.get().getInstalledApps(userId);

// Get specific app info
AppInfo appInfo = NyxBoxCore.get().getAppInfo(packageName, userId);

// Check if app is installed
boolean isInstalled = NyxBoxCore.get().isAppInstalled(packageName, userId);
```

#### App Configuration
```java
// Enable/disable app
NyxBoxCore.get().setAppEnabled(packageName, userId, true);

// Set app permissions
NyxBoxCore.get().setAppPermission(packageName, permission, userId, true);

// Configure app settings
NyxBoxCore.get().setAppSetting(packageName, setting, value, userId);
```
...
NyxBoxCore.get().startVirtualProcess(packageName, userId);
NyxBoxCore.get().stopVirtualProcess(packageName, userId);

// Monitor process status
ProcessInfo processInfo = NyxBoxCore.get().getProcessInfo(packageName, userId);
```

#### Memory Management
```java
// Optimize memory usage
NyxBoxCore.get().optimizeMemory(userId);

// Clear unused resources
NyxBoxCore.get().clearUnusedResources(userId);
```
...
NyxBoxCore.setDebugMode(true);

// Set log level
Slog.setLogLevel(Slog.LEVEL_DEBUG);

// Enable specific debug features
NyxBoxCore.enableDebugFeature("webview", true);
NyxBoxCore.enableDebugFeature("gms", true);
```
...
```java
// Main entry point
NyxBoxCore core = NyxBoxCore.get();
