...
import com.nyxbox.core.system.BProcessManagerService;
import com.nyxbox.core.system.ISystemService;
import com.nyxbox.core.system.ProcessRecord;
...
import com.nyxbox.entity.pm.InstalledPackage;
import com.nyxbox.utils.AbiUtils;
import com.nyxbox.utils.FileUtils;
import com.nyxbox.utils.Slog;
import com.nyxbox.utils.compat.PackageParserCompat;
...
public class BPackageManagerService extends IBPackageManagerService.Stub implements ISystemService {
    ...
    public ApplicationInfo getApplicationInfo(String packageName, int flags, int userId) {
        ...
        if (Objects.equals(packageName, NyxBoxCore.getHostPkg())) {
            try {
                return NyxBoxCore.getPackageManager().getApplicationInfo(packageName, flags);
    ...
    public List<ResolveInfo> queryIntentServices(
        ...
        final String resolvedType = intent.resolveTypeIfNeeded(NyxBoxCore.getContext().getContentResolver());
    ...
    public PackageInfo getPackageInfo(String packageName, int flags, int userId) {
        ...
        if (Objects.equals(packageName, NyxBoxCore.getHostPkg())) {
            try {
                return NyxBoxCore.getPackageManager().getPackageInfo(packageName, flags);
            } catch (PackageManager.NameNotFoundException e) {
                e.printStackTrace();
            }
            return null;
    ...
    private InstallResult installPackageAsUserLocked(String file, InstallOption option, int userId) {
        ...
        try {
            ...
            if (option.isFlag(InstallOption.FLAG_URI_FILE)) {
                ...
                InputStream inputStream = NyxBoxCore.getContext().getContentResolver().openInputStream(Uri.parse(file));
            ...
            PackageInfo packageArchiveInfo = NyxBoxCore.getPackageManager().getPackageArchiveInfo(apkFile.getAbsolutePath(), 0);
            ...
            String hostPackageName = NyxBoxCore.getHostPkg();
            ...
            if (!support) {
                String msg = packageArchiveInfo.applicationInfo.loadLabel(NyxBoxCore.getPackageManager()) + "[" + packageArchiveInfo.packageName + "]";
                return result.installError(packageArchiveInfo.packageName,
                        msg + "\n" + (NyxBoxCore.is64Bit() ? "The box does not support 32-bit Application" : "The box does not support 64-bit Application"));
            }
            ...
            if (option.isFlag(InstallOption.FLAG_SYSTEM)) {
                aPackage.applicationInfo = NyxBoxCore.getPackageManager().getPackageInfo(aPackage.packageName, 0).applicationInfo;
            }
    ...
    public boolean isAppRunning(String packageName, int userId) {
        List<ActivityManager.RunningAppProcessInfo> processes = ((ActivityManager) NyxBoxCore.getContext().getSystemService("activity")).getRunningAppProcesses();
        if (processes == null) {
            return false;
        }
