...
public class LauncherActivity extends Activity {
    ...
    public static void launch(Intent intent, int userId) {
        try {
            ...
            splash.setClass(NyxBoxCore.getContext(), LauncherActivity.class);
            ...
            NyxBoxCore.getContext().startActivity(splash);
    ...
    private PackageInfo getPackageInfoWithFallback(String packageName, int userId) {
        try {
            ...
            return NyxBoxCore.getBPackageManager().getPackageInfo(packageName, 0, userId);
        } catch (Exception e) {
            Slog.w(TAG, "Failed to get package info for " + packageName + " (attempt 1): " + e.getMessage());
            
            try {
                
                return NyxBoxCore.getBPackageManager().getPackageInfo(packageName, 
                ...
                try {
                    ...
                    android.content.pm.ApplicationInfo appInfo = NyxBoxCore.getBPackageManager()
    ...
    private void launchAppAsync(final Intent launchIntent, final int userId) {
        new Thread(() -> {
            try {
                ...
                NyxBoxCore.getBActivityManager().startActivity(launchIntent, userId);
