...
import com.nyxbox.android.app.ActivityThreadAppBindDataContext;
import com.nyxbox.android.app.BRActivity;
import com.nyxbox.android.app.BRActivityManagerNative;
import com.nyxbox.android.app.BRActivityThread;
import com.nyxbox.android.app.BRActivityThreadActivityClientRecord;
import com.nyxbox.android.app.BRActivityThreadAppBindData;
import com.nyxbox.android.app.BRActivityThreadNMR1;
import com.nyxbox.android.app.BRActivityThreadQ;
import com.nyxbox.android.app.BRContextImpl;
import com.nyxbox.android.app.BRLoadedApk;
import com.nyxbox.android.app.BRService;
import com.nyxbox.android.app.LoadedApk;
import com.nyxbox.android.content.BRBroadcastReceiver;
import com.nyxbox.android.content.BRContentProviderClient;
import com.nyxbox.android.graphics.BRCompatibility;
import com.nyxbox.android.security.net.config.BRNetworkSecurityConfigProvider;
...
import com.nyxbox.app.dispatcher.AppServiceDispatcher;
import com.nyxbox.core.CrashHandler;
import com.nyxbox.core.IBActivityThread;
import com.nyxbox.core.IOCore;
import com.nyxbox.core.NativeCore;
import com.nyxbox.core.env.VirtualRuntime;
import com.nyxbox.fake.frameworks.BXposedManager;
import com.nyxbox.metacore.RemoteManager;
import org.lsposed.lsparanoid.Obfuscate;
import com.nyxbox.core.system.user.BUserHandle;
import com.nyxbox.entity.AppConfig;
import com.nyxbox.entity.am.ReceiverData;

import com.nyxbox.fake.delegate.AppInstrumentation;
import com.nyxbox.fake.delegate.ContentProviderDelegate;

import com.nyxbox.fake.hook.HookManager;
import com.nyxbox.fake.service.HCallbackProxy;
import com.nyxbox.utils.Reflector;
import com.nyxbox.utils.SafeContextWrapper;
import com.nyxbox.utils.GlobalContextWrapper;
import com.nyxbox.utils.Slog;
import com.nyxbox.utils.compat.ActivityManagerCompat;
import com.nyxbox.utils.compat.BuildCompat;
import com.nyxbox.utils.compat.ContextCompat;
import com.nyxbox.utils.compat.StrictModeCompat;
...
public class BActivityThread extends IBActivityThread.Stub {
    ...
    private final Handler mH = NyxBoxCore.get().getHandler();
    ...
    public static int getCallingBUid() {
        return getAppConfig() == null ? NyxBoxCore.getHostUid() : getAppConfig().callingBUid;
    }
    ...
    public Service createService(ServiceInfo serviceInfo, IBinder token) {
        ...
        try {
            Context context = NyxBoxCore.getContext().createPackageContext(
    ...
    public void bindApplication(final String packageName, final String processName) {
        if (Looper.myLooper() != Looper.getMainLooper()) {
            ...
            NyxBoxCore.get().getHandler().post(() -> {
    ...
    private Object createBindApplicationData(String packageName, String processName) {
        try {
            ...
            PackageInfo packageInfo = NyxBoxCore.getBPackageManager().getPackageInfo(packageName, PackageManager.GET_PROVIDERS, getUserId());
    ...
    public synchronized void handleBindApplication(String packageName, String processName) {
        ...
        PackageInfo packageInfo = NyxBoxCore.getBPackageManager().getPackageInfo(packageName, PackageManager.GET_PROVIDERS, BActivityThread.getUserId());
        ApplicationInfo applicationInfo = packageInfo.applicationInfo;
        ...
        Object boundApplication = BRActivityThread.get(NyxBoxCore.mainThread()).mBoundApplication();
        ...
        if (NyxBoxCore.get().isHideXposed()) {
        ...
        try {
            ...
            BRActivityThread.get(NyxBoxCore.mainThread())._set_mInitialApplication(mInitialApplication);
            ContextCompat.fix((Context) BRActivityThread.get(NyxBoxCore.mainThread()).getSystemContext());
    ...
    private android.content.pm.ApplicationInfo getApplicationInfo(Object data) {
        try {
            ...
            String packageName = NyxBoxCore.getAppPackageName();
            if (packageName != null) {
                PackageInfo packageInfo = NyxBoxCore.getBPackageManager().getPackageInfo(packageName, 0, getUserId());
    ...
    private Context createFallbackContext(String packageName) {
        try {
            Context baseContext = NyxBoxCore.getContext();
            if (baseContext == null) {
                Slog.e(TAG, "NyxBoxCore.getContext() is null, cannot create fallback context");
                return null;
            }
    ...
    private void setApplication(Application application) {
        try {
            mInitialApplication = application;
            BRActivityThread.get(NyxBoxCore.mainThread())._set_mInitialApplication(application);
            ...
            for (ProviderInfo providerInfo : providers) {
                try {
                    if (processName.equals(providerInfo.processName) ||
                        ...
                        installProvider(NyxBoxCore.mainThread(), context, providerInfo, null);
                    }
    ...
    public static Context createPackageContext(ApplicationInfo info) {
        try {
            return NyxBoxCore.getContext().createPackageContext(info.packageName,
                    Context.CONTEXT_INCLUDE_CODE | Context.CONTEXT_IGNORE_SECURITY);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public IBinder getActivityThread() {
        return BRActivityThread.get(NyxBoxCore.mainThread()).getApplicationThread();
    }
    ...
    public IBinder acquireContentProviderClient(ProviderInfo providerInfo) throws RemoteException {
        ...
        for (String auth : split) {
            ContentProviderClient contentProviderClient = NyxBoxCore.getContext()
    ...
    public void finishActivity(final IBinder token) {
        mH.post(() -> {
            Map<IBinder, Object> activities = BRActivityThread.get(NyxBoxCore.mainThread()).mActivities();
    ...
    public void handleNewIntent(final IBinder token, final Intent intent) {
        mH.post(() -> {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP_MR1) {
                newIntent = BRReferrerIntent.get()._new(intent, NyxBoxCore.getHostPkg());
            } else {
                newIntent = intent;
            }
            Object mainThread = NyxBoxCore.mainThread();
            if (BRActivityThread.get(NyxBoxCore.mainThread())._check_performNewIntents(null, null) != null) {
    ...
    public void scheduleReceiver(ReceiverData data) throws RemoteException {
        ...
        mH.post(() -> {
            ...
            try {
                ...
                NyxBoxCore.getBActivityManager().finishBroadcast(data.data);
    ...
    public static Activity getActivityByToken(IBinder token) {
        Map<IBinder, Object> iBinderObjectMap =
                BRActivityThread.get(NyxBoxCore.mainThread()).mActivities();
    ...
    private void onBeforeCreateApplication(String packageName, String processName, Context context) {
        for (AppLifecycleCallback appLifecycleCallback : NyxBoxCore.get().getAppLifecycleCallbacks()) {
        ...
        try {
            ...
            try {
                ...
                if (validContext == null) {
                    validContext = NyxBoxCore.getContext();
                }
            } catch (Exception e) {
