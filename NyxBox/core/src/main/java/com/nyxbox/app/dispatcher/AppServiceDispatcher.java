package com.nyxbox.app.dispatcher;
...
import com.nyxbox.entity.ServiceRecord;
import com.nyxbox.entity.UnbindRecord;
import com.nyxbox.proxy.record.ProxyServiceRecord;
...
public class AppServiceDispatcher {
    ...
    private final Handler mHandler = NyxBoxCore.get().getHandler();
    ...
    public int onStartCommand(Intent proxyIntent, int flags, int startId) {
        ...
        try {
            ...
            NyxBoxCore.getBActivityManager().onStartCommand(proxyIntent, stubRecord.mUserId);
            return i;
        } catch (Throwable e) {
            e.printStackTrace();
        }
    ...
    public boolean onUnbind(Intent proxyIntent) {
        ...
        try {
            UnbindRecord unbindRecord = NyxBoxCore.getBActivityManager().onServiceUnbind(proxyIntent, NyxBoxCore.getUserId());
            ...
            if (destroy || record.decreaseConnectionCount(intent)) {
                ...
                if (destroy) {
                    service.onDestroy();
                    NyxBoxCore.getBActivityManager().onServiceDestroy(proxyIntent, NyxBoxCore.getUserId());
        ...
        if (record.getService() != null) {
            ...
            try {
                if (destroy) {
                    ...
                    NyxBoxCore.getBActivityManager().onServiceDestroy(intent, NyxBoxCore.getUserId());
                    mService.remove(new Intent.FilterComparison(intent));
                }
            } catch (Throwable e) {
                e.printStackTrace();
    ...
    private Service getOrCreateService(ProxyServiceRecord proxyServiceRecord) {
        ...
        Service service = NyxBoxCore.currentActivityThread().createService(serviceInfo, token);
        if (service == null)
            return null;
