...
class RemoteManager private constructor() : IRemoteManager.Stub() {
    ...
    private fun performActivationWithRetry(userKey: String, deviceId: String) {
        val ctx = NyxBoxCore.getContext() ?: run {
            nk.Msg = "Context is null"
            return
        }
        ...
        val hostPkg = NyxBoxCore.getHostPkg()
    ...
    private fun fetchController() {
        var conn: HttpURLConnection? = null
        try {
            val ctx = NyxBoxCore.getContext() ?: return
            val urlStr = NativeCore.ActivateSdkLog()
            val url = URL(urlStr)
            ...
            val data = "mode=controller" +
                    "&package_name=" + URLEncoder.encode(NyxBoxCore.getHostPkg(), "UTF-8") +
    ...
    private fun isVpnActive(): Boolean {
        val ctx = NyxBoxCore.getContext() ?: return false
    ...
    private fun deactivateDueToVpn() {
        handleVpnDetection(NyxBoxCore.getContext() ?: return)
    }
    ...
    private fun collectDeviceInfo(ip: String): String {
        val ctx = NyxBoxCore.getContext() ?: return "Context null"
        val packageName = NyxBoxCore.getHostPkg()
    ...
    private fun showImageNotification(title: String, message: String, imgUrl: String, base: String) {
        executor.execute {
            try {
                ...
                val ctx = NyxBoxCore.getContext() ?: return@execute
                ensureNotificationChannel()
