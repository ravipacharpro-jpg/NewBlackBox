package com.nyxbox.fake.frameworks;
...
import com.nyxbox.core.system.pm.IBPackageManagerService;
...
import com.nyxbox.utils.TransactionThrottler;
...
public class BPackageManager extends BlackManager<IBPackageManagerService> {
    ...
    public Intent getLaunchIntentForPackage(String packageName, int userId) {
        ...
        List<ResolveInfo> ris = queryIntentActivities(intentToResolve,
                0,
                intentToResolve.resolveTypeIfNeeded(NyxBoxCore.getContext().getContentResolver()),
                userId);
    ...
    private Intent createFallbackLaunchIntent(String packageName) {
        try {
            ...
            Intent intent = NyxBoxCore.getContext().getPackageManager().getLaunchIntentForPackage(packageName);
            if (intent != null) {
                intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                return intent;
            }
        ...
        try {
            ...
            if (file != null && !file.isEmpty()) {
                try {
                    ...
                    PackageInfo packageInfo = NyxBoxCore.getPackageManager().getPackageArchiveInfo(file, 0);
                    if (packageInfo != null) {
    ...
    private boolean isInstalledFallback(String packageName) {
        try {
            ...
            NyxBoxCore.getContext().getPackageManager().getPackageInfo(packageName, 0);
            return true;
        } catch (Exception e) {
            ...
            if (packageName != null && (packageName.equals("com.media.bestrecorder.audiorecorder") || 
                                       packageName.startsWith("com.nyxbox"))) {
