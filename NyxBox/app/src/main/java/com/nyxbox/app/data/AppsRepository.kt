...
import com.nyxbox.app.bean.AppInfo
import com.nyxbox.app.bean.InstalledAppBean
import com.nyxbox.app.util.MemoryManager
...
class AppsRepository {
    ...
    private fun safeLoadAppLabel(applicationInfo: ApplicationInfo): String {
        return try {
            NyxBoxCore.getPackageManager().getApplicationLabel(applicationInfo).toString()
        } catch (e: Exception) {
    ...
    private fun safeLoadAppIcon(
        ...
        return try {
            ...
            val icon = NyxBoxCore.getPackageManager().getApplicationIcon(applicationInfo)
        ...
        try {
            synchronized(mInstalledList) {
                val installedApplications: List<ApplicationInfo> =
                        NyxBoxCore.getPackageManager().getInstalledApplications(0)
                ...
                for (installedApplication in installedApplications) {
                    try {
                        ...
                        if (NyxBoxCore.get().isBlackBoxApp(installedApplication.packageName)) {
                            Log.d(
                                    TAG,
    ...
    fun getInstalledAppList(
        ...
        try {
            ...
            synchronized(mInstalledList) {
                val blackBoxCore = NyxBoxCore.get()
    ...
    fun installApk(source: String, userId: Int, resultLiveData: MutableLiveData<String>) {
        try {
            ...
            if (source.contains("blackbox") ||
                ...
                try {
                    val blackBoxCore = NyxBoxCore.get()
                    val hostPackageName = NyxBoxCore.getHostPkg()
    ...
    fun unInstall(packageName: String, userID: Int, resultLiveData: MutableLiveData<String>) {
        try {
            NyxBoxCore.get().uninstallPackageAsUser(packageName, userID)
    ...
    fun launchApk(packageName: String, userId: Int, launchLiveData: MutableLiveData<Boolean>) {
        try {
            val result = NyxBoxCore.get().launchApk(packageName, userId)
    ...
    fun clearApkData(packageName: String, userID: Int, resultLiveData: MutableLiveData<String>) {
        try {
            NyxBoxCore.get().clearPackage(packageName, userID)
