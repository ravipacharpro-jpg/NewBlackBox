...
import com.nyxbox.utils.TrieTree;
...
public class IOCore {
    ...
    public void enableRedirect(Context context) {
        ...
        try {
            ApplicationInfo packageInfo = NyxBoxCore.getBPackageManager().getApplicationInfo(packageName, PackageManager.GET_META_DATA, NyxBoxCore.getUserId());
            int systemUserId = NyxBoxCore.getHostUserId();
            ...
            File profilesCurDir = new File(profilesRoot, String.format("cur/%d/%s", NyxBoxCore.getUserId(), packageName));
            File profilesRefDir = new File(profilesRoot, String.format("ref/%d/%s", NyxBoxCore.getUserId(), packageName));
            FileUtils.mkdirs(profilesCurDir.getAbsolutePath());
            FileUtils.mkdirs(profilesRefDir.getAbsolutePath());
            rule.put(String.format("/data/misc/profiles/cur/%d/%s", NyxBoxCore.getUserId(), packageName), profilesCurDir.getAbsolutePath());
            rule.put(String.format("/data/misc/profiles/ref/%d/%s", NyxBoxCore.getUserId(), packageName), profilesRefDir.getAbsolutePath());

            if (NyxBoxCore.getContext().getExternalCacheDir() != null && context.getExternalCacheDir() != null) {
                File external = BEnvironment.getExternalUserDir(NyxBoxCore.getUserId());
    ...
    private void proc(Map<String, String> rule) {
        int appPid = NyxBoxCore.getAppPid();
