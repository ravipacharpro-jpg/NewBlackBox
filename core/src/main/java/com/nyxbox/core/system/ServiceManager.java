...
import com.nyxbox.core.system.pm.BXposedManagerService;
...
public class ServiceManager {
    ...
    public static void initBlackManager() {
        NyxBoxCore.get().getService(ACTIVITY_MANAGER);
        NyxBoxCore.get().getService(JOB_MANAGER);
        NyxBoxCore.get().getService(PACKAGE_MANAGER);
        NyxBoxCore.get().getService(STORAGE_MANAGER);
        NyxBoxCore.get().getService(USER_MANAGER);

        NyxBoxCore.get().getService(ACCOUNT_MANAGER);
        NyxBoxCore.get().getService(LOCATION_MANAGER);
        NyxBoxCore.get().getService(NOTIFICATION_MANAGER);
        NyxBoxCore.get().getService(XPOSED_MANAGER);
    }
}
