...
public class AppJobServiceDispatcher {
    ...
    JobService getJobService(int jobId) {
        synchronized (mJobRecords) {
            ...
            try {
                JobRecord record = NyxBoxCore.getBJobManager().queryJobRecord(NyxBoxCore.getAppProcessName(), jobId);
                record.mJobService = NyxBoxCore.currentActivityThread().createJobService(record.mServiceInfo);
