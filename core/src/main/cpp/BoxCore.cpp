...
void install_mprotect_hook() {
    ...
    if (mprotect_ptr) {
        if (DobbyHook(mprotect_ptr, (void*)mprotect_hook, (void**)&original_mprotect) == 0) {
            __android_log_print(ANDROID_LOG_INFO, "NyxBox", "mprotect hook installed (delayed)");
        } else {
            __android_log_print(ANDROID_LOG_WARN, "NyxBox", "mprotect hook failed");
        }
    }
}
...
void install_anogs_hooks() {
    ...
    if (func) {
        if (DobbyHook(func, (void*)anogs_ioctl_hook, (void**)&original_anogs_ioctl) == 0) {
            __android_log_print(ANDROID_LOG_INFO, "NyxBox", "ANOGS Ioctl hook installed (delayed)");
        } else {
            __android_log_print(ANDROID_LOG_WARN, "NyxBox", "ANOGS hook failed");
        }
    } else {
        __android_log_print(ANDROID_LOG_WARN, "NyxBox", "ANOGS symbol not found, skipping");
    }
}
...
int posix_spawn_hook(pid_t *pid, const char *path,
    ...
    __android_log_print(ANDROID_LOG_DEBUG, "NyxBox", "posix_spawn blocked: %s", path ? path : "(null)");
    errno = EPERM;
    return -1;
}
...
void install_posix_spawn_hook() {
    ...
    if (ptr) {
        if (DobbyHook(ptr, (void*)posix_spawn_hook, (void**)&original_posix_spawn) == 0) {
            __android_log_print(ANDROID_LOG_INFO, "NyxBox", "posix_spawn hook installed (delayed)");
        } else {
            __android_log_print(ANDROID_LOG_WARN, "NyxBox", "posix_spawn hook failed");
        }
    } else {
        __android_log_print(ANDROID_LOG_WARN, "NyxBox", "posix_spawn not found");
    }
}
// ==========================================
